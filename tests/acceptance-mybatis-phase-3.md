...
- Then
  - 当前 `ResultSet` 被关闭
  - 当前 SQL 对应的缓存语句被淘汰并关闭
  - 下一次相同 SQL 执行时重新创建新的 `PreparedStatement`
...
- Then
  - 执行器继续尝试关闭剩余语句
  - 最终抛出首个关闭异常
  - 异常消息包含失败语句的 `statementId`、资源路径、SQL 和执行器类型
...
- 验证方式
  ...
  - 在异常路径下断言失败语句会从缓存中淘汰，并在下一次查询时重新创建
  - 在关闭失败路径下断言异常消息包含完整语句上下文
- 边界
  - 不验证跨事务连接复用
  - 不验证缓存命中统计接口
  - 不验证动态 SQL
